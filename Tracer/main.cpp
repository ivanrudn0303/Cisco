...
int EditCopy(HWND proc)
{
	...
	{
		if (strstr((char*)buf, "id") != nullptr)
		{
			int step = atoi(strstr((char*)buf, "id") + 2);
			if (step + Iterator > MaxSize)
			{
				const char *str = "Less";
				HGLOBAL hMem;
